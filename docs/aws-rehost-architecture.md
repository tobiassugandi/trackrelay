...
The guarded RDS switch is now prepared as `make aws-rds-deploy`. It is permitted only after the host-local frozen workload has been collected. The SSM payload contains the deterministic database identifier, but no endpoint, secret ARN, or password. On EC2, the instance role discovers RDS metadata, retrieves its one allowed secret, URL-encodes the credential, and writes a TLS-required SQLAlchemy URL to the mode-`0600` runtime file. It stops and removes the host PostgreSQL container without deleting its volume, migrates RDS, recreates the API, then proves ingestion and persistence across an API restart. Only the resolved PostgreSQL minor and non-secret capacity configuration enter session evidence.

The next local slice will automate the four core correctness scenarios and compact reconciliation evidence against this RDS-backed deployment. A real AWS plan still waits until that validation path is prepared, the private CLI session is authenticated, and the account owner approves cloud session 1's exact resource list, estimate, duration, and cost ceiling.

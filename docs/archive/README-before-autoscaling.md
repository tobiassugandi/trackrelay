# Historical README — before the autoscaling report

Archived on 2026-09-04 to preserve the original setup and implementation notes.
This is historical documentation: its future-tense milestones and status sections
are not the current project status. Start with the [current README](../../README.md)
...
The Terraform root module in `infra/terraform` uses mutually exclusive `rehost` and `async` deployment modes around a shared private RDS/VPC foundation. The former creates the historical x86_64 EC2 host; the latter creates the service-image repositories, encrypted delivery queue and DLQ, Fargate network and IAM boundaries, public load balancer, observable ECS cluster, bounded task logs, private simulator discovery, immutable task definitions, standalone migration, and gated fixed services without that host. The lifecycle manifest freezes the mode through teardown. The [rehost architecture note](../../docs/aws-rehost-architecture.md) explains the synchronous boundary; the [asynchronous architecture note](../../docs/aws-async-architecture.md) explains the current Stage 9.5 boundary and its explicit public-task-network tradeoff. Initialize the locked provider and run formatting, validation, and mocked plan assertions with:
...
The repository exposes `make aws-plan`, approval-gated `make aws-up`, unconditional `make aws-down`, and `make aws-verify-down`. For session 3, the separately armed `make aws-async-deploy` owns three image publications, immutable runtime registration, the zero-exit migration gate, fixed-service enablement, and convergence; any failure or interrupt stops a remaining migration task and triggers destroy plus native verification. Plans and lifecycle logs stay under the ignored `results/aws-sessions/` tree. The verifier checks Terraform state, the tagging API, and native EC2, EBS, VPC, ELBv2, pending/running ECS tasks, services and task definitions, Cloud Map, CloudWatch Logs, all-service ECR, SQS, IAM, RDS, backup, snapshot, and RDS-managed-secret inventories for all resources currently defined; each later resource type must add its own native check before use.
...
Compose reads `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, and `POSTGRES_PORT` from `.env`, with development defaults when the file is absent. TrackRelay defaults to host port `5433` to avoid conflicting with a separately installed PostgreSQL server; the container itself still listens on the standard port `5432`. SQLAlchemy reads `TRACKRELAY_DATABASE_URL`. The Fargate runtime instead supplies the `TRACKRELAY_DATABASE_{HOST,PORT,NAME,USER,PASSWORD,SSLMODE}` components; TrackRelay URL-encodes them in process so Terraform never constructs a plaintext credential URL. Database infrastructure remains separate from the API routes; readiness will use it in Step 1.9.
...
The local synchronous implementation is complete and measured through Step 8.6. Its frozen reference sustains 250 events/s on the recorded machine and first fails at 500 events/s. Phase 9 cloud sessions 1 and 2 validated the synchronous RDS-backed rehost and completed the controlled 2.5× hardware-flexibility result; both stacks were fully torn down. Stage 9.4 is locally complete: it defines the queue boundary, adds the separately runnable SQS worker, makes retry and replay safe, closes durable acceptance with a transactional PostgreSQL outbox, and freezes machine-checkable end-to-end completion and queue-drain guardrails. Stage 9.5 now has separately built and locally tested service images plus its encrypted registry, SQS/DLQ, Fargate network, IAM, load balancer, ECS cluster, bounded logs, private discovery, digest-pinned task definitions, standalone migration, and gated one-task service definitions. Mutually exclusive deployment modes prevent its plan from also creating the historical EC2 host, and an armed multi-phase controller now freezes image digests, gates services on a successful migration, verifies convergence, and cleans up failures. The next increment adds the experiment metrics and locally tested session-3 integration runner before any approved cloud integration.

...
- [ ] For cloud session 3 only, immediately follow successful convergence with `make aws-async-integration` using the same repeated approval values. Let it own the exact 1-, 10-, and 100-event workloads, continuous request and drain evidence, reconciliation, representative CloudWatch evidence, complete destroy, and native absence verification. Do not run a second workload or teardown controller in parallel.

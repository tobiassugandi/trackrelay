...
.PHONY: sync test test-integration lint run run-worker run-downstream image-api image-api-smoke image-worker image-worker-smoke image-simulator image-simulator-smoke images images-smoke rehost-config rehost-smoke generate reconcile summary scenario-normal scenario-duplicate scenario-out-of-order scenario-downstream-outage load-smoke load-prepare load-ramp load-slow load-outage load-baseline elasticity-prepare elasticity-driver-check aws-check aws-plan aws-up aws-async-deploy aws-async-integration aws-fixed-control aws-rehost-publish aws-rehost-deploy aws-rehost-workload aws-rds-deploy aws-rds-deploy-canary aws-rds-correctness aws-scaling-prepare aws-scaling-run-tier aws-scaling-transition aws-scaling-report aws-scaling-canary aws-scaling-session aws-down aws-verify-down infra-init infra-check db-up db-status db-check db-down migrate migration-status
...
aws-async-deploy:
	$(UV) run --locked trackrelay-aws-async-deploy \
		--session-id "$(SESSION_ID)" \
		--profile "$(TRACKRELAY_AWS_PROFILE)" \
		--region "$(TRACKRELAY_AWS_REGION)" \
		--api-ingress-cidr "$(API_INGRESS_CIDR)" \
...
aws-async-integration:
	$(UV) run --locked trackrelay-aws-async-integration \
		--session-id "$(SESSION_ID)" \
		--profile "$(TRACKRELAY_AWS_PROFILE)" \
		--region "$(TRACKRELAY_AWS_REGION)" \
		--api-ingress-cidr "$(API_INGRESS_CIDR)" \
...
aws-fixed-control:
	$(UV) run --locked trackrelay-aws-fixed-control \
		--session-id "$(SESSION_ID)" \
		--profile "$(TRACKRELAY_AWS_PROFILE)" \
		--region "$(TRACKRELAY_AWS_REGION)" \
		--api-ingress-cidr "$(API_INGRESS_CIDR)" \
...
aws-rds-deploy-canary:
	$(UV) run --locked trackrelay-aws-rehost deploy-rds-canary \
		--session-id "$(SESSION_ID)" \
		--profile "$(TRACKRELAY_AWS_PROFILE)" \
		--region "$(TRACKRELAY_AWS_REGION)" \
		--api-ingress-cidr "$(API_INGRESS_CIDR)" \
...
aws-scaling-canary:
	$(UV) run --locked trackrelay-aws-vertical-scaling-canary \
		--session-id "$(SESSION_ID)" \
		--profile "$(TRACKRELAY_AWS_PROFILE)" \
		--region "$(TRACKRELAY_AWS_REGION)" \
		--api-ingress-cidr "$(API_INGRESS_CIDR)" \

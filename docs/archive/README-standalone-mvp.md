> Historical README snapshot of the standalone MVP, preserved on 16 September 2026.
> For the current result, see the [repository README](../../README.md).
...
![TrackRelay architecture: courier updates pass through a load balancer and fixed-capacity API into a PostgreSQL transactional outbox and SQS queue. Fargate workers deliver events downstream, with CloudWatch demand and unfinished-work signals controlling worker autoscaling.](../assets/trackrelay-architecture.png)
...
![Offered traffic rises to 25 events per second; running workers expand from one to eight, then return to one as demand falls.](../assets/autoscaling/demand-workers.png)
...
![Ingestion p95 is mostly low, with one ten-second spike to 572 milliseconds. Unfinished deliveries peak at 479 and clear while peak traffic continues.](../assets/autoscaling/latency-backlog.png)
...
- [Evidence, chart data and measurement notes](../autoscaling-report.md)
- [Architecture](../aws-async-architecture.md) · [Implementation history](../implementation-plan.md)
- [Run the AWS demo](../aws-elasticity-diagnostic-runbook.md) — requires a fresh plan, explicit budget approval and teardown.
- [Earlier setup and development notes](../archive/README-before-autoscaling.md) — archived technical reference.

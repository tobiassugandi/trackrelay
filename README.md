...
Local ingestion currently publishes the versioned event-ID-only job to a deterministic process-local recording queue. This keeps the boundary runnable and inspectable without a local AWS emulator, but it is intentionally not durable or connected to a running worker. A process restart can discard recorded jobs, and a database commit followed by a publish failure remains an explicit `503` gap. Later Stage 9.4 steps replace the fake with SQS and define durable acceptance so a successful fast response cannot hide lost work.

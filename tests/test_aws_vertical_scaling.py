...
def test_current_tier_runner_preserves_evidence_and_stops_at_first_failure(
    ...
    def runtime_starter(_session, *, point, **_kwargs):
        nonlocal runtime_start_count
        start = datetime(2026, 8, 29, 14, tzinfo=UTC) + timedelta(
            minutes=runtime_start_count * 4
        )
    ...
    for evidence_name in (
        ...
        assert len(tuple(tier_root.glob(f"rates/*/*/{evidence_name}"))) == len(
            executed_trials
        )
    assert len(tuple(tier_root.rglob("post-trial-reset.json"))) == len(
        executed_trials
    )

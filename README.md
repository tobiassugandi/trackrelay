...
The repository exposes `make aws-plan`, approval-gated `make aws-up`, unconditional `make aws-down`, and `make aws-verify-down`. Plans and lifecycle logs stay under the ignored `results/aws-sessions/` tree. The verifier checks Terraform state, the tagging API, and native EC2, EBS, VPC, ELBv2, active ECS, Cloud Map, CloudWatch Logs, all-service ECR, SQS, IAM, RDS, backup, snapshot, and RDS-managed-secret inventories for all resources currently defined; each later resource type must add its own native check before use.

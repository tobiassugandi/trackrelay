...
The Terraform root module in `infra/terraform` retains the rehost host and private RDS data layer—one x86_64 EC2 instance, a private single-AZ PostgreSQL instance, their disposable network, RDS-managed credentials, and SSM access without SSH—and now also defines the three service-image repositories, encrypted delivery queue and DLQ, Fargate network and IAM boundaries, public load balancer, observable ECS cluster, bounded task logs, private simulator discovery, immutable task definitions, standalone migration, and gated fixed services. The [rehost architecture note](docs/aws-rehost-architecture.md) explains the synchronous boundary; the [asynchronous architecture note](docs/aws-async-architecture.md) explains the current Stage 9.5 boundary and its explicit public-task-network tradeoff. Initialize the locked provider and run formatting, validation, and mocked plan assertions with:

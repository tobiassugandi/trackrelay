...
The Terraform root module in `infra/terraform` retains the rehost host and private RDS data layer—one x86_64 EC2 instance, a private single-AZ PostgreSQL instance, their disposable network, RDS-managed credentials, and SSM access without SSH—and now also defines the three service-image repositories plus the encrypted delivery queue and DLQ foundation. The [rehost architecture note](docs/aws-rehost-architecture.md) explains the synchronous boundary, earlier ARM cloud-session result, and current x86_64 Stage 9.3 revision. Initialize its locked provider and run formatting, validation, and mocked plan assertions with:
...
The repository exposes `make aws-plan`, approval-gated `make aws-up`, unconditional `make aws-down`, and `make aws-verify-down`. Plans and lifecycle logs stay under the ignored `results/aws-sessions/` tree. The verifier checks Terraform state, the tagging API, and native EC2, EBS, VPC, all-service ECR, SQS, IAM, RDS, backup, snapshot, and RDS-managed-secret inventories for all resources currently defined; each later resource type must add its own native check before use.

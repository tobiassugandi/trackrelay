...
The transport-neutral worker behavior is developed locally as a separate application service. It receives a validated message, checks whether PostgreSQL already records a successful delivery, and acknowledges such a replay without repeating the downstream call. Otherwise, it reloads the authoritative event by ID, invokes downstream delivery with durable attempt recording, and acknowledges the message only after that operation succeeds. Loading, delivery, attempt-recording, and acknowledgement failures propagate without a successful acknowledgement so SQS can make the message visible again.

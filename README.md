...
Local ingestion defaults to publishing the versioned event-ID-only job to a deterministic process-local recording queue. This keeps the boundary runnable and inspectable without a local AWS emulator, but it is intentionally not durable or connected to a running worker. A process restart can discard recorded jobs, and a database commit followed by a publish failure remains an explicit `503` gap. The later durable-acceptance step must still close that gap so a successful fast response cannot hide lost work.

The transport-neutral worker behavior is now developed locally as a separate application service. It receives a validated message, reloads the authoritative event from PostgreSQL by ID, invokes downstream delivery with durable attempt recording, and acknowledges the message only after that operation succeeds. Loading, delivery, attempt-recording, and acknowledgement failures propagate without a successful acknowledgement so the eventual SQS message can become visible again. An acknowledgement failure after successful downstream delivery is intentionally treated as ambiguous; retry and duplicate-effect handling are completed by the later retry, DLQ, and guardrail steps.

The real AWS path is available behind the same boundaries. Set `TRACKRELAY_DELIVERY_QUEUE_BACKEND=sqs`, `TRACKRELAY_SQS_QUEUE_URL`, and `TRACKRELAY_AWS_REGION` for the API publisher, then run the worker separately with `make run-worker`. The worker long-polls in batches, validates each job lazily so one malformed message does not prevent valid neighbors from being processed, and maps successful acknowledgement to SQS `DeleteMessage`. `TRACKRELAY_SQS_VISIBILITY_TIMEOUT_SECONDS` must exceed the downstream timeout multiplied by `TRACKRELAY_SQS_MAX_MESSAGES`, preventing a sequentially processed batch from becoming visible again before its legitimate processing window ends. No AWS emulator or substitute local broker is required; adapter tests use deterministic SQS clients, and live integration remains reserved for approved cloud session 3.

...
The API, one-off migration, host-local PostgreSQL, and downstream simulator runtime is defined separately in `deploy/rehost/compose.yaml`. The next Stage 9.2 slice will add a guarded deployment mode that replaces that host-local database connection with the private RDS endpoint. See the [rehost architecture note](../../docs/aws-rehost-architecture.md).

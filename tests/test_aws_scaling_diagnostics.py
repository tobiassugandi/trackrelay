...
def test_retains_high_resolution_data_and_both_alarm_histories(tmp_path):
    ...
    assert len(calls) == 4
    queries = loads(calls[0][calls[0].index("--metric-data-queries") + 1])
    assert {q["MetricStat"]["Metric"]["MetricName"] for q in queries} == {
        "ArrivalRate",
        "OutstandingEvents",
    }
    assert all(
        q["MetricStat"]["Period"] == 10 and q["MetricStat"]["Stat"] == "Maximum"
        for q in queries
    )
    assert "--include-not-scaled-activities" in calls[1]
    assert calls[2][calls[2].index("--alarm-name") + 1].endswith("-demand-high")
    assert calls[3][calls[3].index("--alarm-name") + 1].endswith("-release-safe")
